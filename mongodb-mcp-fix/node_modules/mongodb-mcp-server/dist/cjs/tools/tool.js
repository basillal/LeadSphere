...
exports.formatUntrustedData = formatUntrustedData;
...
class ToolBase {
    ...
    get toolMeta() {
        ...
        let maxRequestPayloadBytes = constants_js_1.TRANSPORT_PAYLOAD_LIMITS[transport] ?? constants_js_1.TRANSPORT_PAYLOAD_LIMITS.stdio;
    ...
    async invoke(args, { signal, requestInfo }) {
        ...
        try {
            ...
            this.session.logger.error({
                id: logger_js_1.LogId.toolExecuteFailure,
                context: "tool",
    ...
    constructor({ name, category, operationType, session, config, telemetry, elicitation, uiRegistry, }) {
        this.name = name;
    ...
    disable() {
        if (!this.registeredTool) {
            this.session.logger.warning({
                id: logger_js_1.LogId.toolMetadataChange,
    ...
    verifyAllowed() {
        ...
        if (errorClarification) {
            this.session.logger.debug({
                id: logger_js_1.LogId.toolDisabled,
                context: "tool",
    ...
    async appendUIResource(result) {
        ...
        if (this.uiRegistry) {
            ...
            uiResource = (0, server_1.createUIResource)({
                uri: `ui://${this.name}`,
                content: {
                    type: "rawHtml",
                    htmlString: uiHtml,
 ...
 */
function formatUntrustedData(description, ...data) {
    const uuid = (0, getRandomUUID_js_1.getRandomUUID)();
