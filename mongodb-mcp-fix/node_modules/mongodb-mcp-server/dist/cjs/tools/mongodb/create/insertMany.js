...
const manageNestedFieldPaths_js_1 = require("../../../helpers/manageNestedFieldPaths.js");
const zSupportedEmbeddingParametersWithInput = mongodbSchemas_js_1.zSupportedEmbeddingParameters.extend({
    input: zod_1.z.array(zod_1.z.object({}).passthrough()).describe(`\
...
`),
    ...
    async replaceRawValuesWithEmbeddingsIfNecessary({ database, collection, documents, embeddingParameters, }) {
        ...
        for (const input of embeddingParameters.input) {
            for (const fieldPath of Object.keys(input)) {
                if (!vectorIndexes.some((index) => index.path === fieldPath)) {
                    throw new errors_js_1.MongoDBError(errors_js_1.ErrorCodes.AtlasVectorSearchInvalidQuery, `Field '${fieldPath}' cannot be used with embeddingParameters because it does not have a classic vector search index (type: 'vector') configured for manual embedding generation in collection ${database}.${collection}. This field either has no vector search index, or it has an auto-embed index (type: 'autoEmbed') where MongoDB automatically generates embeddings at indexing time. Use the collection-indexes tool to verify the index configuration for this field. If it has an auto-embed index, remove it from embeddingParameters and provide the raw text directly in the document instead.`);
                }
            }
        }
        ...
        for (const [index, { fieldPath, documentIndex }] of flattenedEmbeddingsInput.entries()) {
            ...
            (0, manageNestedFieldPaths_js_1.setFieldPath)(processedDocuments[documentIndex], fieldPath, generatedEmbeddings[index]);
        }
