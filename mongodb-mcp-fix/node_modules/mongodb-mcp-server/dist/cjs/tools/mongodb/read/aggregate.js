...
exports.AggregateTool = exports.getAggregateArgs = exports.pipelineDescriptionWithVectorSearch = void 0;
...
exports.pipelineDescriptionWithVectorSearch = `\
...
const getAggregateArgs = (vectorSearchEnabled) => ({
    pipeline: zod_1.z
        ...
        .describe(vectorSearchEnabled ? exports.pipelineDescriptionWithVectorSearch : genericPipelineDescription),
});
...
`),
    ...
    async countAggregationResultDocuments({ provider, database, collection, pipeline, abortSignal, }) {
        ...
        for (const stage of pipeline) {
            if ("$vectorSearch" in stage) {
                ...
                if (!vectorSearchStage.queryVector) {
                    throw new errors_js_1.MongoDBError(errors_js_1.ErrorCodes.AtlasVectorSearchInvalidQuery, "Either 'queryVector' (for classic vector indexes) or 'query' (for auto-embed indexes) must be provided in $vectorSearch. Use the collection-indexes tool to verify which type of index the target field has.");
                }
                if (!vectorSearchStage.embeddingParameters) {
                    throw new errors_js_1.MongoDBError(errors_js_1.ErrorCodes.AtlasVectorSearchInvalidQuery, "embeddingParameters is mandatory when providing queryVector as a string for classic vector search indexes (type: 'vector'). If the target field has an auto-embed index (type: 'autoEmbed'), use 'query' instead of 'queryVector'. Use the collection-indexes tool to verify the index type.");
                }
    ...
    resolveTelemetryMetadata(args, { result }) {
        ...
        const usesVectorSearch = maybeVectorStage !== null && maybeVectorStage instanceof Object && "$vectorSearch" in maybeVectorStage;
        if (usesVectorSearch &&
...
AggregateTool.toolName = "aggregate";
