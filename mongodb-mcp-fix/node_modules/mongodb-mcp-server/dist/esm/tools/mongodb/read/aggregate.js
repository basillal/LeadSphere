            ...
            if (this.config.indexCheck) {
                ...
                switch (usesVectorSearchIndex) {
                    ...
                    case "non-existent-index":
                        throw new MongoDBError(ErrorCodes.AtlasVectorSearchIndexNotFound, `Could not find an index with name "${indexName}" in namespace "${database}.${collection}".`);
                    case "valid-index":
        ...
        for (const stage of pipeline) {
            if ("$vectorSearch" in stage) {
                ...
                if (!vectorSearchStage.queryVector) {
                    throw new MongoDBError(ErrorCodes.AtlasVectorSearchInvalidQuery, "Either 'queryVector' (for classic vector indexes) or 'query' (for auto-embed indexes) must be provided in $vectorSearch. Use the collection-indexes tool to verify which type of index the target field has.");
                }
                if (!vectorSearchStage.embeddingParameters) {
                    throw new MongoDBError(ErrorCodes.AtlasVectorSearchInvalidQuery, "embeddingParameters is mandatory when providing queryVector as a string for classic vector search indexes (type: 'vector'). If the target field has an auto-embed index (type: 'autoEmbed'), use 'query' instead of 'queryVector'. Use the collection-indexes tool to verify the index type.");
                }
                ...
                if (!embeddings) {
                    throw new MongoDBError(ErrorCodes.AtlasVectorSearchInvalidQuery, "Failed to generate embeddings for the query vector.");
                }
