...
import { commaSeparatedToArray, getExportsPath, getLogPath, oneWayOverride, onlyLowerThanBaseValueOverride, onlySubsetOfBaseValueOverride, parseBoolean, } from "./configUtils.js";
...
export const configRegistry = z4.registry();
const ServerConfigSchema = z4.object({
    ...
    connectionString: z4
        ...
        .register(configRegistry, { isSecret: true, overrideBehavior: "not-allowed" }),
    loggers: z4
        .preprocess((val) => commaSeparatedToArray(val), z4.array(z4.enum(["stderr", "disk", "mcp"])))
        .check(z4.minLength(1, "Cannot be an empty array"), z4.refine((val) => new Set(val).size === val.length, {
    ...
    disabledTools: z4
        .preprocess((val) => commaSeparatedToArray(val), z4.array(z4.string()))
        .default([])
    ...
    vectorSearchSimilarityFunction: z4
        ...
        .register(configRegistry, { overrideBehavior: "override" }),
    previewFeatures: z4
        .preprocess((val) => commaSeparatedToArray(val), z4.array(z4.enum(previewFeatureValues)))
        .default([])
    ...
    healthCheckPort: z4
        ...
        .optional()
        .describe("Port number for the healthCheck HTTP server (only used when transport is 'http'). If provided, `healthCheckHost` must also be set.")
        .register(configRegistry, { overrideBehavior: "not-allowed" }),
    healthCheckHost: z4
        .string()
        .optional()
        .describe("Host address to bind the healthCheck HTTP server to (only used when transport is 'http'). If provided, `healthCheckPort` must also be set.")
        .register(configRegistry, { overrideBehavior: "not-allowed" }),
});
...
export const ALL_CONFIG_KEYS = Object.keys(UserConfigSchema.shape);
